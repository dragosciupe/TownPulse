...
import calendarStyles from'../components/EventCalendarButton.module.css';
...
function EventsCalendarPage() {
  ...
  const filteredByCity = loadedEvents.filter(event => event.city===userData?.city)
  const [isActive,setIsActive] =useState(0)
  const [filteredEvents, setFilteredEvents] = useState(filteredByCity);
  function handleSelect(timestamp,number){
    setFilteredEvents(filteredByCity.filter(event=>event.date===timestamp))
    setIsActive(number);
  ...
  return (
    <>
      <h1 className={clasess.title}>Welcome to the events calendar page</h1>
      <div className={clasess.calendarDiv}>
      <EventCalendarButton isActive={isActive===1 } timestamp={day1} onSelect={()=>handleSelect(day1,1)} />
      <EventCalendarButton isActive={isActive===2 } timestamp={day2T} onSelect={()=>handleSelect(day2T,2)} />
      <EventCalendarButton isActive={isActive===3 } timestamp={day3T} onSelect={()=>handleSelect(day3T,3)} />
      <EventCalendarButton isActive={isActive===4 } timestamp={day4T} onSelect={()=>handleSelect(day4T,4)} />
      <EventCalendarButton isActive={isActive===5 } timestamp={day5T} onSelect={()=>handleSelect(day5T,5)} />
      <EventCalendarButton isActive={isActive===6 } timestamp={day6T} onSelect={()=>handleSelect(day6T,6)} />
      <EventCalendarButton isActive={isActive===7 } timestamp={day7T} onSelect={()=>handleSelect(day7T,7)} />
      </div>
